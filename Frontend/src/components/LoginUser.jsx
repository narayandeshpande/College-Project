...
const LoginUser = () => {
  ...
  const onSubmit = async (data) => {
    ...
    await axios
      ...
      .catch((errors) => {
        toast.error(errors.response?.data?.error || "An error occurred");
      });
  };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <input
              type="email"
              id="email"
              className="w-full px-4 py-2 border rounded-lg bg-gray-800 text-white placeholder-white focus:outline-none focus:ring-2 focus:ring-blue-500"
